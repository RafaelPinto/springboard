...
-- There are five facilities that do not charge to members
...
FROM (
    ...
    surname,
    firstname,
    STR_TO_DATE(joindate, '%Y-%m-%d %H:%i:%s') AS joindate_timestamp
    FROM Members
    ) as sub
WHERE joindate_timestamp = (
    SELECT
    MAX(STR_TO_DATE(joindate, '%Y-%m-%d %H:%i:%s'))
    FROM Members
    )
...
SELECT
      ...
      (SELECT CONCAT(firstname, " ",surname)
       FROM Members AS m
       WHERE m.memid = b.memid) AS fullname,
       CASE WHEN b.memid = 0 THEN (f.guestcost * b.slots)
       ELSE (f.membercost * b.slots) END AS total_cost
FROM Bookings as b
INNER JOIN Facilities AS f
